...
連続空間、離散時間を扱う。$$A \ge 2$$個のマルチエージェントシステムを考える。制御できる車両の位置を$$\mathbf{x}_{t}^r = \mathbf{x}_{t}^1 \in \mathbb{R}^{A \times d}$$、制御できないすべての車両の位置を$$\mathbf{x}_{t}^h = \mathbf{x}_{t}^{2:A} \in \mathbb{R}^{(A-1) \times d}$$とする。小文字は実現値、大文字は確率変数とする。

計画のため次の情報が使うことできる。

* 観測$$\mathbf{o} \doteq \{\mathbf{x}_0, \mathbf{i}_0\}$$
  * 現在時刻$$t=0$$のすべてのエージェントの2次元位置$$\mathbf{x}_{0} \in \mathbb{R}^{A \times d}$$
  * LIDARやカメラなどの高次元の環境に関する情報$$\mathbf{i}_0$$
* 自車両が向かうべき2次元位置のゴール$$\mathbf{g}$$およびゴールへ向かう移動可能領域$$\delta_{\mathbb{G}}(\mathbf{x})$$ （$$\delta_{\mathbb{G}}(\mathbf{x})$$は$$\mathbf{x}$$が領域内ならば1、そうでなければ0となる。$$\mathcal{G} = (\mathbf{g}, \mathbb{G})$$とする。）

CfOは制御できない他車両の不確実性を考慮した期待値$$\mathcal{L}_{CfO}$$を最大化することで、マルチエージェントの相互作用を考慮した自車両の計画(軌道)$$\mathbf{x}_{\le T}^{r}$$を求める。
...
* 第１項$$\log q_{\theta}(\bar{\mathbf{x}}_{\le T} \mid \mathbf{o})$$はすべてのエージェントの軌道$$\bar{\mathbf{x}}_{\le T} $$が学習したエキスパートの軌道に近いかを示す尤度である。$$q_{\theta}$$はそのエキスパートの軌道を模倣する生成モデルである。Behavioral Modelと呼ぶ。 
* 第２項は自車両の計画の最後の位置がゴールにどれだけ近いかを示す尤度である。
* 第３項はゴールへ向かうための移動可能領域に関するペナルティである。例えば道路外や他車両との衝突である。数値計算上$$\delta_{\mathbb{G}}(\bar{\mathbf{x}}_{\le T}) = 0$$のときには大きな負の値を使う。
* 他車両は制御できないので、生成モデルの潜在変数を基本分布からサンプリングする。

以下ではBehavioral Modelについて説明する。

### Behavioral Model

Behavioral Modelとしてマルチエージェントの軌道を予測する[PRECOG](../PRECOG: PREdiction Conditioned On Goals in Visual Multi-Agent Settings/summary.md)のESPと同様のものを使う。ただしセンサは俯瞰図方式ではなくレンジイメージでCNNに入力する。すなわちBehavioral Modelは自己回帰生成モデルを用いる。

マルチエージェントの軌道を模倣する行動モデル$$q(\mathbf{X} \mid \phi)$$は過去の状態及び観測を受け取るネットワークによる自己回帰確率分布モデルの積に分解できる。

$$q(\mathbf{X}_{\le T}^{1:A} = \mathbf{x}_{\le T}^{1:A} \mid \mathbf{o})= \prod_{t=1}^T \prod_{a=1}^A q^a(\mathbf{X}_t^a = \mathbf{x}_t^a ; \phi_t^a)$$

$$\phi_t^a$$は過去の状態および観測を受け取るネットワークが出力したパラメータ$$\phi_t^a=f_{\theta}^a(\mathbf{x}_{\le T}^{1:A}; \mathbf{o})$$である。$$q^a$$は事前に設定された単純な基本分布$$\bar{q}^a$$からサンプルした$$\mathbf{z}_t^a \in \mathbb{R}^d$$を$$\mathbf{x}_t^a$$に微分可能かつ可逆な変換することによってサンプルできる確率分布とする。そしてこのような変換関数を$$\mathbf{x}_t^a = m(\mathbf{z}_t^a; \phi_t^a)$$とする。例えば基本分布$$\bar{q}^a$$を正規分布とすると、$$m$$と$$\phi_t^a$$は

$$m(\mathbf{z}_t^a; \phi_t^a) = \mu_t^a + \sigma_t^a \mathbf{z}_t^a$$

$$\phi_t^a = \{ \mu_t^a, \sigma_t^a \}$$

である。$$\mu_t^a$$、$$\sigma_t^a$$は状態$$\mathbf{X}_t^a$$における平均と標準偏差となる。また変換関数$$m$$は$$\mathbf{z}_t^a$$と$$\theta$$によってパラメータ化される決定的な方策$$\pi$$として表すことができる。

$$\mathbf{x}_{t}^{a} = \pi^a (\mathbf{x}_{<t}^{1:A}, \mathbf{o}; \mathbf{z}_t^a, \theta)$$

この模倣モデルの訓練に用いるデータセットはゴールに到達するために有効な行動のモードを多く含んでいると仮定する。有効な行動としては右左折、受動的な走行、能動的な走行などである。例えば前方車両の速度が遅く追い越しをすることで快適になるシナリオの場合、データセットに積極的に追い越しをするデータを含んでいる必要がある。
...
| Noncontingent, $$\mathcal{L}^{\mathbf{r}}$$     | 目的関数に$$\mathcal{L}^{\mathbf{r}}$$を用いたunderconfidentプランナー （補足参照） |
| Noncontingent, $$\mathcal{L}^{\mathbf{joint}}$$ | 目的関数に$$\mathcal{L}^{\mathbf{joint}}$$を用いたoverconfientプランナー（補足参照） |
...
がある。これらのシステムは予測と計画を異なるモジュールで行い、マルチエージェント環境で動作する。しかし、このモジュール構成は予測の正確性に依存している。

しかしながら、これらの予測はロボットの行動に対して他のエージェントがどのように反応するか、他のエージェントの行動によってロボットの行動がどう変化するか(共依存性)を無視している。このアプローチは"frozen robot problem"として知られる自信不足の行動につながる可能性がある。これらから予測と行動を切り離して考えるのは行動計画の組み立てとして不適切である。

予測と行動を同時に行う、つまりこの共依存性を考慮する方法がContingency Planning(不測の事態に備えた計画)である。Contingency Planningに関する研究は例えば以下のものがある。
...
### Human leader planningとJoint planning
