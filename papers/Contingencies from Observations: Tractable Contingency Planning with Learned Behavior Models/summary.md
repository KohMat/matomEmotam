...
と表せる。$$\mu_t^a$$、$$\sigma_t^a$$は状態$$\mathbf{X}_t^a$$における平均と標準偏差となる。
...
End-to-Endシステム(例えば[Deep Imitative Models](../DEEP IMITATIVE MODELS FOR FLEXIBLE INFERENCE, PLANNING, AND CONTROL/summary.md))が最近の研究で注目されている。これは生センサの情報からマルチエージェント環境下でナビゲーションを行う方法である。しかしながらこれらの方法はその他のエージェントの行動をモデル化しないため、条件３を満たさない。
