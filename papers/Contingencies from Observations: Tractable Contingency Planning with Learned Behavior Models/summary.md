...
右折、左折、追い越しなどの車両が相互に協力するような運転シーンに対応できる経路計画方法CfO (Contingencies from Observations)を提案する。CfOは環境中にいる他の車両の行動の不確実性を考慮して経路を計画する（Contingency Planning([補足](#Contingency planningについて))）。

CfOは行動モデルを使って他の車両の行動の不確実性に対応する。CfOで用いる行動モデルは生成モデルの一種であるautoregressive flowモデルである。モデルは右折、左折、追い越しなどの運転データの分布に近くなるように訓練される。学習済みの行動モデルは車両の将来の経路を様々なパターンで生成する。また経路の尤度を計算する。例えば自動運転車が交差点を右折するシナリオで次の予測パターンを生成することが期待できる。

* 自動運転車と合流先の車線上の車の両方がアグレッシブに運転する
* 自動運転車と合流先の車線上の車の両方がパッシブに運転する
* 一つはアグレッシブでもう一つはパッシブに運転する

CfOは生成した各予測パターンを評価関数で評価する。評価関数は自動運転車が安全にゴールへ到達するためのハンドクラフトした関数である。そして評価値の期待値が最大になるような経路を選ぶ。つまり、一つの将来の結果に基づく最適な経路ではなく、将来の行動がわからない他の車両に対応するため、複数の行動に安全に対応できる一つの経路を求める。これによって下図の下の段のように相手の車に応じて柔軟に動作する。合流先の車線上の車がアグレッシブであれば、自動運転車は道を譲って車が交差点を通過するのを待つ（図の左下）。合流先の車線上の車がパッシブであれば、自動運転車は道を譲られているので、交差点を通過する（図の右下）。
...
行動モデルの具体的なネットワークアーキテクチャはマルチエージェントの軌道を予測するautoregressive flowモデルESPを使う。ESPは[PRECOG: PREdiction Conditioned On Goals in Visual Multi-Agent Settings](../PRECOG: PREdiction Conditioned On Goals in Visual Multi-Agent Settings/summary.md)で提案されたモデルである。ただし観測としてLIDARの点群を用いる場合、点群を上空から見た俯瞰図方式ではなくレンジイメージがCNNに入力される。ESPのアーキテクチャ図を示す。
