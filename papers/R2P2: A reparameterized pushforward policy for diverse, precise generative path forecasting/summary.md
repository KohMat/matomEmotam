...
この問題を解決する方法として、$$H(p, q_{\pi})$$と逆の性質を持つ訓練データ$$p$$のモデル$$q_{\pi}$$に対するクロスエントロピー$$H(q_{\pi}, p)$$を組み合わせた対称クロスエントロピーを学習に使う方法がある。モードのカバーと品質の向上を行うことができるが、$$H(q_{\pi}, p)$$は微分不可能であるため、そのままでは評価することができないという別の問題がある。
