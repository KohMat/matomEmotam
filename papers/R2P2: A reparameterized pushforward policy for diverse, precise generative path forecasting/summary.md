...
ただし、$$\tilde{p}$$は訓練データ分布を近似したネットワークである。都合上、後のセクションで説明する。以上より微分可能かつ可逆なシミュレーター$$g_{\pi}(z; \phi) : \mathbb{R}^{T \times 2} \rightarrow \mathbb{R}^{T \times 2}$$で将来の軌道分布をモデル化することができれば、対称クロスエントロピーを使った訓練を行うことができる。

![simulator](./simulator.png)

### シミュレーターの設計

将来の軌道分布をモデル化にあたって初期時刻から前時刻$$t-1$$までの車両の位置$$x_{t-1}$$および観測$$\phi$$から次の時刻の車両の位置$$x_t$$への遷移確率を正規分布として仮定する。
...
交差点上で自動車がランダムに直進、右折、左折するシミュレーター上でR2P2 RNNを訓練し、検証を行った。検証結果が次のとおりである。表よりR2P2 RNNは他の手法の性能を上回っていることが確認できる。
