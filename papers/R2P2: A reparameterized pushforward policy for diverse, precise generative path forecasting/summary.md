...
この問題を解決する方法として、$$H(p, q_{\pi})$$と逆の性質を持つ訓練データ$$p$$のモデル$$q_{\pi}$$に対するクロスエントロピー$$H(q_{\pi}, p)$$を組み合わせた対称クロスエントロピーを学習に使う方法がある。2つのエントロピーの利点を両立した訓練を行うことができるが、$$H(q_{\pi}, p)$$は微分不可能であるため、そのままでは評価することができないという別の問題がある。
...
本論文は$$H(q_{\pi}, p)$$の評価を可能にする将来軌道の分布の新しいモデリング方法を提案し、実験でその有効性を示した。また他の$$H(q_{\pi}, p)$$を評価する方法であるREINFORCEより提案手法が低分散の勾配推定値を計算することができることを示した。
...
> ただし$$J_{g_{\pi}}(g_{\pi}^{-1}(z; \phi))$$はヤコビアンである。

この関係を使うと対称クロスエントロピーは基本分布$$q_0$$とシミュレータ$$g_{\pi}(z; \phi)$$を用いた計算可能な式に変形できる。
...
ただし、$$\tilde{p}$$は訓練データ分布を近似したネットワークである。都合上、$$\tilde{p}$$は後のセクションで説明する。以上より微分可能かつ可逆なシミュレーター$$g_{\pi}(z; \phi) : \mathbb{R}^{T \times 2} \rightarrow \mathbb{R}^{T \times 2}$$で将来の軌道分布をモデル化することができれば、対称クロスエントロピーを使った訓練を行うことができる。

![simulator](./simulator.png)

### シミュレーターの設計

対称クロスエントロピーが使用可能となるように将来の軌道分布のモデル化するにあたって初期時刻から前時刻$$t-1$$までの車両の位置$$x_{t-1}$$および観測$$\phi$$から次の時刻の車両の位置$$x_t$$への遷移確率を正規分布として仮定する。
...
この仮定を使うと、Reparametrization trickによって微分可能かつ$$\sigma_t^{\pi} = 0$$を除き可逆な自己回帰型シミュレーターが次式で定義できる。
...
また通常高次元のヤコビアン行列式の計算量は$$O(N^3)$$であるため重たいが、自己回帰型の特性によりこのシミュレーターのヤコビアンは非常に簡素な次の式で表される。

$$\log |\det J_{g_{\pi}}(g_{\pi}^{-1}(z; \phi))| = \sum_t \log | \det(\sigma_t^{\pi}(\psi_t; \theta)) |$$

最終的にクロスエントロピー$$H(p, q_{\pi})$$は次の式に変形できる。
...
本論文ではR2P2を実現する３つのネットワークアーキテクチャを提案する。
...
各時間における自車両の位置を図に示すようなL個のセルで構成される有限領域で離散化することで訓練データの分布の近似を行う。

![p_approximate](./p_approximate.png)

各タイムステップにおける位置を独立と仮定すると訓練データの分布は次式で表すことができる。

$$\tilde{p} = \prod_t \tilde{p}_c(x_t \mid \phi)$$

ここで$$\tilde{p}_c$$は時刻$$t$$における車両の位置に対応するL個のカテゴリカルな分布を出力するネットワークである。３層のCNNで構成される。ボクセル化されたLIDARの点群俯瞰図および道路のクラス情報$$M \in \mathbb{R}^{H_{map} \times W_{H_{map}} \times C}$$を入力として、各グリッドの確率を出力する。ネットワーク$$\tilde{p}_c$$の訓練はロジスティクス回帰を使って行われる。
...
交差点上で自動車がランダムに直進、右折、左折するシミュレーター上でR2P2 RNNを訓練し、検証を行った。検証結果が次の図と表である。図および表よりR2P2 RNNは他の手法の性能を上回っていることが確認できる。

![simple_cross_test_result](./simple_cross_test_result.png)

### CaliForecastingとKITTIデータセット

作成したデータセットCaliForecastingおよびKITTI上でR2P2を検証した。検証結果が次の表である。検証結果より比較手法と比べてR2P2の結果が良いことがわかる。これはR2P2のアプローチが有効性であることを示す。また$$\beta=0$$のR2P2に対して$$\beta>0$$のR2P2の性能が良いことから、$$H(q_{\pi}, \tilde{p})$$を使った学習がサンプルの質をあげること示している。
