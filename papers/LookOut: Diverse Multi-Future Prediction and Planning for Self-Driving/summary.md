...
Object Detectorは２つの浅いCNNヘッダーからなるネットワークである。Backbone CNNで計算した特徴マップから多クラスの物体分類と検出したアクターの位置とヘディング、サイズを出力する。そして検出したアクターの情報を元にBackboneで計算した特徴マップからRotated RoI Alignを行い、アクターの特徴マップを抽出する。
...
これらの４つのネットワークは物体検出損失と運動予測損失からなるマルチタスク損失を使ってEnd-to−Endで訓練される。物体検出損失はクラスごとにハードネガティブマイニングを使用したバイナリークロスエントロピー損失とHuber損失で構成される。Object Detectorの多クラス分類ヘッダーと回帰ヘッダーに対して計算される。運動予測損失はHuber損失による再構成誤差とKLダイバージェンスからなる損失である。Prediction decoderに対して計算される。
