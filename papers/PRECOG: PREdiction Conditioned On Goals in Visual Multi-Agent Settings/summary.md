...
**CARLAおよびnuScenes**：CALRAおよびnuScenesから10個のデータセットを作成し予測性能を検証した。ESP, no LIDARは観測からLIDARを除いたESPである。ESP, RoadはnuScenesの道路領域をバイナリマスクで表現した入力を追加したESPである。ESP, flexは可変数のエージェントに対応するESPである。CALRAおよびnuScenesのデータセットでESPの性能が比較手法のの性能を上回った。

![esp_performance](./esp_performance.png)

### PRECOGの検証

CALRAおよびnuScenesを使いPRECOGの予測性能を検証した。Planingを行うエージェントは自車両のみとした。各データの最後の位置をゴールとして設定した。ゴールの尤度は正規分布を用いた。検証結果は次のとおりである。自動運転車は$$\hat{m}_K$$である。その他のエージェントは$$\hat{m}_K^{a=1,2,3,4,5}$$である。表ではその他のエージェントは自動運転車から距離が近い順に表示されている。

![precog_result](./precog_result.png)

ESPは[DESIRE](https://arxiv.org/abs/1704.04394)に比べて結果が良い。PRECOGはESPより良い結果を示している。自車両$$\hat{m}_K^1$$だけでなくその他のエージェントの予測性能が向上している。一番近い車両$$\hat{m}_K^2$$の結果が最も影響を受けていることから、PRECOGが有効であることを示している。
