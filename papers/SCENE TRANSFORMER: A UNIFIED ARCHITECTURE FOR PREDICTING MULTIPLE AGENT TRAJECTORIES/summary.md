...
各経路の確率スコアを計算するため、エンコーダーの最初のSelf-Attentionを行った後、特徴量の平均値を計算する。そしてその特徴量$$(F, 1, 1, D)$$を元の特徴量$$(F, A, T, D)$$に連結し、特徴量$$(F, A+1, T+1, D)​4$を作成する。その後、デコーダーにて連結した特徴量をスライスし、２層のMLPを使ってF個の将来に対するlogitを出力する。
