...
* 車両がレーン追従、レーンチェンジ、右左折、停止など有限数からなる高レベルの行動セットの一つを使うという仮定を置いて予測を行う方法がある。これらは観測された軌跡をもとに現在の行動を検出する。そのため、検出された高レベル行動が有効な期間だけに制限される。IGP2では車両の行動を高レベル行動セットからなるをシーケンスとして捉えることで、より車両の予測を説明可能かつ長期間を行うことができる。
* この車両の予測により交差点やレーンチェンジなど消極的でない速やかな走行が行える。
